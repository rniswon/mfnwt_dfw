...
!     Updates saturation for previous time step.
      ...
      SUBROUTINE GWF2UPW1AD(Iunitdfw, IGRID)
...
!     SUBROUTINE GWF2UPWUPDATE. UPDATE VALUES AFTER OUTER ITERATION.
      SUBROUTINE GWF2UPWUPDATE(Itest,Iunitdfw,Igrid)
...
!     ARGUMENTS
!     ------------------------------------------------------------------
      INTEGER Igrid, ij, ic, ir, il, Itest, i, i1, i2, Iunitdfw, nadfw
!     -----------------------------------------------------------------
C------SET POINTERS FOR THE CURRENT GRID.
      CALL SGWF2UPW1PNT(Igrid)
      IF ( Iunitdfw.GT.0 ) THEN  
      ...
      ELSE
        nadfw = 0
      END IF
      DO ij = nadfw+1, nadfw + Numactive 
        il = Diag(ij- nadfw, 1)
        ir = Diag(ij- nadfw, 2)
        ic = Diag(ij- nadfw, 3)
        IF (Itest==1) HNEW(ic,ir,il) = HITER(ij)
        I1 = IA(ij)
        I2 = IA(ij+1)-1
        DO i = I1, I2
          A(i) = 0.0D0
        END DO
      END DO
      CALL Sn_update(nadfw)
      END SUBROUTINE
!     -----------------------------------------------------------------
!     Updates saturation for latest iteration.

      SUBROUTINE Sn_update(nadfw)
      USE GWFUPWMODULE
